...
const OneBook = () => {
    ...
    const fetchBook = async () => {
        setLoading(true);
        try {
            const res = await ApiCall(`/api/v1/books/${id}`, "GET");
            if (!res?.error) {
                setBook(res.data);
    ...
    const fetchRelatedBooks = async (id) => {
        try {
            const res = await ApiCall(`/api/v1/books/by-subject/${id}`, "GET");
            if (!res?.error && res.data) {
                // Filter books by same subject or author
                const related = res.data
                    .filter(b => b.id !== parseInt(id))
                    .slice(0, 4); // Get 4 related books
